    ...
    try {
        ...
        for (let idx = 0; idx < posts.length; idx++) {
            ...
            if (post.isActive && timePassed < dayInSec) {
                ...
                card.className = `plan-card echo-post ${rank.class}`;
                card.id = `post-${i}`;
                ...
                authorSpan.className = 'echo-author';
                authorSpan.textContent = `${author.slice(0, 6)}...${author.slice(-4)}`;

                const badge = document.createElement('span');
                badge.className = 'nft-badge';
                badge.textContent = `${userBalance} NFT`;

                authorInfo.appendChild(rankIcon);
                authorInfo.appendChild(authorSpan);
                authorInfo.appendChild(badge);

                const timer = document.createElement('span');
                timer.className = 'echo-timer';
                timer.id = `timer-${i}`;
                timer.dataset.expiry = String(postTimestamp + dayInSec);
                ...
                progressBar.className = 'echo-progress-bar';
        ...
        if (fragment.childNodes.length === 0) {
            ...
            empty.textContent = 'No active echoes.';
