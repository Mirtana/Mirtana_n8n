const MIRTA_CONFIG = window.MirtanaProtocolConfig.MIRTA_CONFIG;
...
async function sendMirtaTokens() {
    ...
    try {
        ...
        // Отправляем в n8n, используя уже существующие переменные chainId и config
        await sendWeb3EventToN8n(
            await signer.getAddress(), // Адрес отправителя
            tx.hash,
            `Transfer: ${amount} MIRTA`,
            config.address, // Адрес контракта токена
            {
                type: 'TRANSFER',
                ...
                amount: amount,
                token: 'MIRTA',
                chainId: chainId // <-- Добавили ID сети
            }
        );
